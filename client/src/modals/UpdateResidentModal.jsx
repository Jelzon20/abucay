...
const UpdateResidentModal = ({ resident, onClose, onUpdated }) => {
  const [formData, setFormData] = useState({
    ...resident,
    birthday: resident.birthday
      ? new Date(resident.birthday).toISOString().split("T")[0]
      : "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/residents/updateResident/${resident._id}`, {
        method: "PUT",
