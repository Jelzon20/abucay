...
const AddLuponModal = ({ show, onClose, onSubmit }) => {
  const [form, setForm] = useState({
    defendant: "",
    complainant: "",
    description: "",
    scheduleOfHearing: null,
    status: "",
  });
  ...
  const handleDateChange = (e) => {
    setForm({ ...form, scheduleOfHearing: e.target.value }); // Format: YYYY-MM-DD
  };
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    try {
      const res = await fetch("/api/lupons/addLupon", {
        method: "POST",
