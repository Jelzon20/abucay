...
import AddResolutionModal from "../modals/AddResolutionModal";
import UpdateResolutionModal from "../modals/UpdateResolutionModal";
import DeleteResolutionModal from "../modals/DeleteResolutionModal";
...
const ResolutionsTab = () => {
  ...
  const columns = [
    { label: "Resolution No", key: "resolutionNo" },
  ...
  const fetchResolutions = async () => {
    setIsLoading(true);
    const res = await fetch("/api/resolutions/getResolutions");
  ...
  const filtered = resolutions.filter((resol) => {
    const fullText = `
      ${resol.resolutionNo}
      ${resol.title}
      ${resol.description}
  ...
  const handleExport = () => {
    ...
    const filename = `resolutions-${datePart} ${timePart}.csv`;
  ...
  const handleDelete = (resol) => {
    setDeleteData(resol);
    setDeleteModalOpen(true);
  };

  const handleUpdate = (resol) => {
    setEditData(resol);
      ...
      <div className="max-w-full mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 space-y-6">
            ...
            <table className="min-w-full bg-white dark:bg-gray-800">
              ...
              <tbody>
                {paginated.map((resol) => (
                  ...
                  >
                    <td className="px-6 py-3">{resol.resolutionNo}</td>
                    <td className="px-6 py-3">{resol.title}</td>
                    <td className="px-6 py-3">{resol.author}</td>
                    <td className="px-6 py-3">
                      {formatDateForInput(resol.dateFiled)}
                    </td>
                    <td className="px-6 py-3">{resol.status}</td>
                    <td className="px-6 py-3 flex gap-2">
                      <button
                        onClick={() => handleUpdate(resol)}
                        className="text-blue-600 hover:text-blue-800 dark:text-blue-300 dark:hover:text-blue-500"
                      >
                        <PencilIcon className="h-5 w-5" />
                      </button>
                      <button
                        onClick={() => handleDelete(resol)}
        ...
        {showAddModal && (
          <AddResolutionModal
            ...
            onSubmit={() => {
              toast.success("New resolution saved!");
              fetchResolutions();
