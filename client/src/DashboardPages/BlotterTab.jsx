...
const BlotterTab = () => {
  ...
  const fetchBlotters = async () => {
    setIsLoading(true);
    const res = await fetch("/api/blotters/getBlotters");
  ...
  const filtered = blotters.filter((blotter) => {
    const fullText = `
      ${blotter.incident}
      ${blotter.description}
      ${blotter.takenBy}
  ...
  const handleExport = () => {
    ...
    const filename = `blotters-${datePart} ${timePart}.csv`;
  ...
  const handleDelete = (blotter) => {
    setDeleteData(blotter);
    setDeleteModalOpen(true);
  };

  const handleUpdate = (blotter) => {
    setEditData(blotter);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
      ...
      <div className="max-w-full mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 space-y-6">
        <div className="flex flex-col md:flex-row justify-between gap-4 items-start md:items-center">
          <Button onClick={() => setShowAddModal(true)}> + Add Blotter</Button>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm">
            <table className="min-w-full bg-white dark:bg-gray-800">
              ...
              <tbody>
                {paginated.map((blotter) => (
                  <tr
                    key={blotter._id}
                    className="border-t hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                  >
                    <td className="px-6 py-3">{blotter.incident}</td>
                    <td className="px-6 py-3">{blotter.description}</td>
                    <td className="px-6 py-3">{blotter.takenBy}</td>
                    <td className="px-6 py-3">
                      {formatDateForInput(blotter.date)}
                    </td>
        ...
        {showAddModal && (
          <AddBlotterModal
            ...
            onSubmit={() => {
              toast.success("New blotter saved!");
              fetchBlotters();
